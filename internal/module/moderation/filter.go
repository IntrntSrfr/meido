...
func newModerationSettingsCommand(m *module) *bot.ModuleCommand {
	return &bot.ModuleCommand{
		...
		Triggers:         []string{"m?moderation settings"},
		Usage:            "m?moderation settings warns [enable / disable]\nm?moderation settings maxwarns [0 - 10]\nm?moderation settings warnduration [0 - 365]",
		Cooldown:         time.Second * 2,
		CooldownScope:    bot.CooldownScopeChannel,
		RequiredPerms:    discordgo.PermissionAdministrator,
		CheckBotPerms:    false,
